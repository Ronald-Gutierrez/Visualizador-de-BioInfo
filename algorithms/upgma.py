...
def get_clustering_levels(dist_matrix, Z):
    ...
    for i, (cluster1, cluster2, dist, _) in enumerate(Z):
        new_cluster_label = cluster_labels[int(
            cluster1)] + cluster_labels[int(cluster2)]
        result += f'Nivel {i+1}: Cluster {"".join(cluster_labels[int(cluster1)])} se une con Cluster {
            "".join(cluster_labels[int(cluster2)])} a una distancia de {dist:.4f}\n'
...
def plot_dendrogram_with_distances(Z, labels, avg_distances, differences, filepath):
    ...
    ddata = dendrogram(Z, labels=labels, leaf_rotation=0,
                       leaf_font_size=12, orientation='right', no_plot=False)
