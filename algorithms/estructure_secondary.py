    ...
    for d in range(1, n):
        for i in range(n - d):
            ...
            if sequence[i] + sequence[j] in ['CG', 'GC', 'AU', 'UA']:
                min_energy = min(min_energy, E[i + 1][j - 1] + alpha[sequence[i] + sequence[j]])
...
def identify_structures(sequence, pairs):
    ...
    unpaired = set(range(1, len(sequence) + 1)) - {i for pair in pairs for i in pair}
...
def plotData(sequence, matches, output_file):
    ...
    nx.draw_networkx_nodes(G, pos, node_color=nx.get_node_attributes(G, "color").values(), node_size=200, edgecolors="#00092C", linewidths=1.5)
    ...
    for structure in structures:
        if len(structure) == 3:
            ...
            if (i, j) not in labeled_positions:
                x = (pos[i][0] + pos[j][0]) / 2
                y = (pos[i][1] + pos[j][1]) / 2
                plt.text(x, y, name, fontsize=8, ha='center', va='center', bbox=dict(facecolor='white', alpha=0.5))
    ...
    for i in range(0, len(sequence), 10):
        plt.annotate(str(i+1), xy=pos[i+1], xytext=(5, 5), textcoords='offset points', arrowprops=dict(arrowstyle="->", color='black'))

    plt.axis('off')
    plt.savefig(output_file)
    plt.close()

# seq = "ucaagcguuagagaagucauuaugugauaaaaaaauucaacuugguaucaacuuaacuaa"
# sequence = seq.upper()
# alpha = {"CG": -1, "GC": -1, "AU": -1, "UA": -1}

# energy_matrix, final_score = predict_secondary_structure(sequence, alpha)
# traceback_structure, paired_positions = traceback(sequence, energy_matrix, alpha)

# sys.stdout = open('output.txt', 'w')

# print("Matriz de Energia:")
# print("\t" + "\t".join(sequence))  
# for i in range(len(energy_matrix)):
#     print(sequence[i] + "\t" + "\t".join(map(str, energy_matrix[i])))  

# print("\nScore Final:", final_score)

# output_image = 'static/result_img/grafico.png'
# plotData(sequence, paired_positions, output_image)
# sys.stdout.close()
